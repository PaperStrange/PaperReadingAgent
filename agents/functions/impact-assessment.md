...
| 核心功能清单 | 11 项（下表） | 每季度复核；运行前自检，与实际不符以实际为准并告警 |
...
1. **运行前自检（必须，不得写死数字）**：
   ...
   - **API 路由数**：打开 `backend/main.py` 统计实际路由装饰器（`@app.get/@app.post` 等）数量与路径，`N_routes` 以**实际统计值**计（≠8 时告警并列出差异）。
   - 自检结论写入报告（`N_core`、`N_routes`、与清单差异）。
2. **盘点变更集**：变更文件按模块归类。
3. **计算 A**：按映射表逐项判定触达；`A = 触达核心功能数 / N_core(自检)`。
4. **计算 B**：变更触及 `backend/main.py` → 保守计 B = N_routes/N_routes = 1.0；否则按各路由宿主模块计 `B = 触达路由数 / N_routes(自检)`。
...
| 4 | 8 条 API 路由+SSE+Broker | backend/main.py | `class RunEventBroker`、`@app.get` |
...
| 9 | GUI 画布与面板 | frontend/src/ | `App.jsx` |
...
| 路由 | 宿主模块 |
|---|---|
| /api/health、/api/new_session、/api/reset_session、/api/session_records/{id}、/api/stream/{sid}/{rid}、/api/translate_preview、/api/run_step（定义）、/api/providers（定义） | backend/main.py（触及即 B=1.0，保守） |
| /api/run_step（执行逻辑） | app/orchestration.py（仅改此 → 1/N_routes） |
| /api/providers（注册表） | provider_config.py（仅改此 → 1/N_routes） |

# 核心文件区域清单（窄档恒包含；编辑点）

- **代码区**：`paper-qa-script/app/*.py`、`backend/main.py`、`paper-qa-script/provider_config.py`、`frontend/src/`、`verify/`、`agents/`、`scripts/agent-ops.py`
