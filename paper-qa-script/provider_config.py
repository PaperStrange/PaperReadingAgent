...
用法：
  1) 环境变量 PAPERQA_PROVIDER=deepseek|dashscope|openai 选择服务商（默认 dashscope）。
  2) 密钥按以下顺序读取：
...
# macOS 仓库默认 dashscope；Windows 仓库默认 deepseek（各自保持一致行为）
DEFAULT_PROVIDER = os.getenv("PAPERQA_PROVIDER", "dashscope").lower()
