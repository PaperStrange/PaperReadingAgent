...
│       └── frontend/                   # React+Vite（App.jsx / FlowNode.jsx / ModelConfigPanel.jsx / DataSourcePanel.jsx / SchemaFieldList.jsx / FunctionTraceNode.jsx / JsonTree.jsx）
...
├── verify/                    # 验收脚本（smoke/e2e/agent/provider_switch/embed_load/remote_e2e/prune_callbacks/index_health + eval_retrieve + gui_check*.mjs）+ e2e 结果
...
- **FlowNode.jsx / ModelConfigPanel.jsx / DataSourcePanel.jsx / SchemaFieldList.jsx / FunctionTraceNode.jsx / JsonTree.jsx**：步骤节点卡片（参数 JSON 编辑——本地草稿态防光标跳、Run/Run Upstream/Run From Here、快照、trace 列表）；Config 节点「模型配置」面板（provider 下拉联动 /api/providers 自动带出 api_base/model/vision/embedding）+「数据源」切换面板（local/remote、URL/arXiv/DOI 列表、manifest 路径）+「配置唯一真源」分组只读清单（Sprint-11 拉取 /api/config_schema，类型/默认值/影响提示/当前值高亮，降级不影响现有面板）；函数节点卡片（文本全文/中文翻译按钮/图片预览）；JSON 树渲染。**样式规范（Sprint-4）**：节点内 block 标题一律 `.node-block-title`、字段标签一律 `.ds-label`、说明文字 `.ds-sub/.ds-hint`，新面板必须复用。
