...
### D-250926-04 决策登记册的 `待回填` 条目 **全量回填**（条数以闸门现跑为准）
...
- 生效状态: 生效（**已完成** 2026-09-26：`V-001`~`V-103` 全量回填 ＋ §3 的 3 条验证时间戳；现值一律以 `verify/verify_decision_register.py` 现跑判决行为准，本行不写死）
- 证据: 侦察口径见本条执行记录；判据 = `verify/verify_decision_register.py`（断言数与 `待回填 N 条` 均以该闸门现跑输出为准，本行不写死）
- 验证时间戳: 2026-09-26（UTC+8，本条全量回填完成时点）
- 违背事故: 无

### D-250926-05 G2 关闭期 S1 硬停条款：**用户单次豁免**（规则原文不修订）

- 原文（逐字）: "需要你裁的其实是三种处置：我很想让你体验惩罚，但是这次算了，按我单次豁免记录"
- 出处: 会话（2026-09-26，回答主代理就复盘 §5 行 14 提出的三选一：甲 补记违背／乙 裁定条款不适用／丙 现在补做一次硬停）
- 时点: 2026-09-26（UTC+8）
- 主代理归纳: 用户选**单次豁免**——本次**不执行** S1 档要求的硬停动作与恢复前的用户二次确认，且**不倒填**硬停记录、**不裁定**条款不适用。豁免范围**仅限本次**；`V-034`/`V-035` 的 S1 条款**原文不改**，同一红线再次触发仍按 S1 处置。复盘 §5 行 14 据此记为"**用户单次豁免**"而**不是"已修"**。
- 生效状态: 生效（**单次豁免**，2026-09-26）
- 证据: 复盘 §5 行 14 处置栏（`docs/iteration/phases/testing-governance/2026-09-26-g2-close-retro.MD`）与闭环台账 A 表行 14；被豁免条款原文见本文件 `V-034`/`V-035`
- 验证时间戳: 2026-09-26（UTC+8，本条登记时点）
- 违背事故: **有**——S1 档要求的硬停与二次确认在 G2 关闭期**确未执行**（R1 于关闭期复现），事实成立且**不因豁免而消失**；用户本次决定不予追究、要求如实记录而不倒填

### D-250926-06 关闭标准＝**零未闭环项**（G2／G3 均按此执行；不回溯 G1）

- 原文（逐字）: ""关闭"以什么为准：G2、G3均按零未闭环项执行，我想看看效果，在相关文档中注意备注时效"
- 出处: 会话（2026-09-27 00:06 UTC+8，回答主代理就"关闭以什么为准"的提问：过闸门即关闭 vs 零未闭环项才关闭）
- 时点: 2026-09-27（UTC+8）
- 主代理归纳: 关闭标准取**严的一把**——**G2 与 G3 均按"零未闭环项"执行**，即关闭时不允许存在任何未处置的发现项，不以"过闸门 ＋ 未达标项留待下批"结案。**时效**：自裁定时刻起对**仍在进行中**的 G2 生效，并适用于 G3；**不回溯 G1**——Sprint-17 是在"按声明关闭"的旧形态下结案的（其 M1 为 7/11 未达标，且当时本闸门体系尚不存在），G1↔G2 的一切对照须标注"两批关闭形态不同"。**计数口径**：复盘 §5 的行全部计入未闭环，除非该行**已由用户裁定处置**（含单次豁免）；"已由用户裁定处置"与"已修"是两件事——**该读法已由用户于 2026-09-27（UTC+8）确认**（用户原话逐字："可以"），**不再是"待更正的临时读法"**。用户同时要求"在相关文档中注意备注时效"，故本口径已同步至 G2 计划 §6 DoD、G2 复盘 §5 表头、闭环台账 §0。
- 生效状态: 生效（2026-09-27 UTC+8 起，适用 G2／G3，不回溯 G1）
- 证据: `docs/iteration/sprint/2026-09-25-governance-batch-plan-G2.MD` §6 新增第 7 条；`docs/iteration/phases/testing-governance/2026-09-26-g2-close-retro.MD` §5 表头；`docs/iteration/phases/testing-governance/2026-09-26-g2-closure-ledger.MD` §0 权威边界表
- 验证时间戳: 2026-09-27（UTC+8，本条登记时点；未闭环条数以复盘 §5 现读为准，本行不写死）
- 违背事故: 无（本条是尺度裁定，不是被违背的规则）；**附带事实**：本条落地时 G2 仍有未闭环项，且其中一部分正是"过闸门即关闭"这一旧读法下会被放过的那类
...
- 出处: docs/iteration/sprint/2026-09-25-governance-batch-plan-G2.MD:125；docs/iteration/sprint/2026-09-25-governance-batch-plan-G2.MD:125；docs/iteration/sprint/2026-09-25-governance-batch-plan-G2.MD:125
- 时点: 2026-09-25
...
- 出处: docs/iteration/sprint/2026-09-25-governance-batch-plan-G2.MD:116
- 时点: 未标注
...
### V-047 D5 TG-10③ 不排期
- 原文（逐字）: "暂不用管也不需要现在就排G3"
- 出处: docs/iteration/sprint/2026-09-25-governance-batch-plan-G2.MD:127
- 时点: 2026-09-25
...
### V-048 用户追问冲突原因
- 原文（逐字）: "为什么会有冲突"
- 出处: docs/iteration/sprint/2026-09-25-governance-batch-plan-G2.MD:134
- 时点: 2026-09-25
...
- 备注: 出处行 docs/iteration/phases/testing-governance/2026-09-25-governance-exit-evidence.MD:117 的「未实现目标」是主代理自己写的小标题用语，用户原话写作"还没实现的目标"（同文件 :114）；本条的 `原文` 已按父代理 2026-09-26 授权**更正为后者**（见该条上方的原文级更正留痕备注），本备注保留以记录更正前后的对照。
