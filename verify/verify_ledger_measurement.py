...
不是测量值——`started_at`/`ended_at` 逐字节相同（3 例，dur 恰 `0.00`）、`dur` 恰为整十分钟（3 例，
恰好 `10.00`）、`rounds=5` 而报告只有 3 轮、60 个 `agents/runs/*` 目录 vs 59 条账本记录、
`doc-audit-009` 终态未写回（账本滞留 `status=running`）。这些数字会被看板与评审直接引用，
**失真的可观测性比没有可观测性更坏**（3-LEARNED 1.59：可观测性必须自证）。

**口径**（唯一真源 = `agents/policy.json::ledger_measurement`，本文件不另写一份；三条可核文本）：

    dur      = `ended_at - started_at`（同一 run 的**累计墙钟**时长；`round` 追加会把 ended_at
    ...
    unknown  **缺值必须显式 `unknown`**：无法测量时 `dur_minutes=null` + `measurement_source=unknown`，
               展示层写 `dur=unknown`；**禁止**用 `0.00`／整十分钟等退化值冒充实测值
               （`0.00` 的含义是"未测得"，不是"零耗时"）。

**断言分组**（哪些 FAIL、哪些 WARN 写死在政策文本 + 本文件的信息串里）：

    A 账本 ↔ `agents/runs/**` **双向**一致：有目录无记录 / 有记录无目录（终态 run 必须有产物目录；
      非终态 run 尚未产出，不算缺失）。历史例外走 `run-dir-exceptions.json` 白名单（**每条必须有
      reason**，且**只减不增**：已不再需要的例外留在表里 = FAIL）。
    B 时间戳非退化：`zero_duration`（两端逐字节相同 / dur 恰 0.00）、`round_duration`（dur 恰为
      `degenerate.round_minutes_multiple` 的整数倍）、`negative_duration`（末态早于起点）、
      `missing_timestamp`（终态 run 缺任一端）→ **新 run 一律 FAIL；历史 run 计入棘轮上限**。
    C `rounds` 与报告轮次一致（报告可数者）→ 新 run FAIL；历史计入棘轮。
    D **终态已写回**：`status` 仍非终态但产物目录里已有终态报告（`terminal_report_globs`）→ FAIL。
    E 产出未测量：`succeeded` 但 `output_chars=0`；评审类 run（spec `scope_required: true`）无
      `result_files` → 新 run FAIL；历史计入棘轮。
    F `measurement_source` 契约（`wall-clock` / `declared` / `unknown`）+ `dur_minutes` 一致性：
      新 run 必须带这两个字段；标 `unknown` 时 `dur_minutes` 必须是 `null`（不得回落 0.00）；
      可测量的终态 run 必须标 `wall-clock`；`dur_minutes` 与时间戳的偏差 > 容差 = 数字被改写。
    G **历史棘轮**：截止日（含）之前的 run 按**计数上限**放行（上限 = 2026-09-25 实测值，写在
      `legacy_ratchet.caps`，只许下调），超过上限逐条点名 FAIL；截止日之后的新 run 无上限、无豁免。
      `review_by` 到期未重评 → FAIL（没有到期日的豁免就是永久豁免）。

**模式**：

    .venv\\Scripts\\python.exe verify\\verify_ledger_measurement.py
        # 默认：真实数据（仓库 agents/）+ 内置自检（干净 fixture 必须过、四类反向对照必须 FAIL）
...
退出码：0=通过；1=检出违规（逐条点名）；2=数据/政策缺失（fail-closed，不静默放行）。
"""
...
# 项目权威时区 UTC+8（与自动 run-id / 产物目录命名同口径，见 scripts/agent-ops.py 的 A5 说明）
...
def _exempted_local(value):
    ...
    仅用于反向对照样本（合成 run 行/目录名）；闸门自身引用的口径、阈值、白名单路径一律来自
...
def is_legacy(run: dict, cutoff: str) -> bool:
    """历史 run = 本地日期 **≤** 截止日；日期不可判定 → 按**新 run** 严格判定（fail-closed）。"""
...
def terminal_report_files(run_dir: Path, globs: tuple[str, ...]) -> list[str]:
    """产物目录里像"终态报告"的文件（用于判"终态已写回"）。"""
    if not run_dir.is_dir():
        return []
    return [str(p.relative_to(run_dir)) for p in sorted(run_dir.rglob("*"))
...
def check_caps_shape(caps: dict, review_by: str, cutoff: str, today: str) -> list[str]:
...
def evaluate(policy, runs: list[dict], runs_dir: Path,
    ...
    problems += check_caps_shape(caps, review_by, cutoff, today)

    findings: list[dict] = []          # 参与棘轮的历史缺陷
    strict: list[dict] = []            # 新 run 缺陷（无上限）
    ...
    def add(kind: str, run: dict | None, detail: str, *, force_strict: bool = False) -> None:
        ...
        if force_strict or run is None or not is_legacy(run, cutoff):
            strict.append(item)
        else:
    ...
    for run in runs:
        ...
        if status in credible and int(run.get("output_chars") or 0) == 0:
            add("output_chars_zero", run, f"status={status} 但 output_chars=0 —— 产出未测量"
                                          f"（不得以 0 冒充『无产出』）")
        if str(run.get("role") or "") in review_roles and not (run.get("result_files") or []):
            add("missing_result_files_review", run,
                f"评审类 run（role={run.get('role')}，spec 声明 scope_required=true）没有 result_files"
                f" —— 评审产物未归档，评审结论不可回溯")

        # F：measurement_source / dur_minutes 契约（**只对新 run 强制**；历史 run 无此字段属棘轮范围）
        #    模型（政策 measurement_sources）：wall-clock = 可测且非退化（dur_minutes 必为实测值）；
        #    declared = 有读数但不是测量值（退化）；unknown = 无从测量。后两者的 dur_minutes **必须为
...
# 合成 run 的样本表（本地日期 2026-09-26 > 截止日 → 走**新 run** 的严格判定）。
# 只有 `_exempted_local` 标注的 fixture 字面量使用角色名，闸门自身政策一律来自 policy.json。
FIXTURE_ROWS = _exempted_local([
    {"run_id": "run-2026-09-26-code-review-901", "role": "code-review", "status": "succeeded",
     "started_at": "2026-09-26T01:00:00+00:00", "ended_at": "2026-09-26T01:37:00+00:00",
     "output_chars": 12345, "result_files": ["runs/run-2026-09-26-code-review-901/code-review.report.md"],
     "rounds_count": 2, "measurement_source": "wall-clock", "dur_minutes": 37.0, "measurement_flags": []},
    {"run_id": "run-2026-09-26-impact-assessment-902", "role": "impact-assessment", "status": "succeeded",
     "started_at": "2026-09-26T02:00:00+00:00", "ended_at": "2026-09-26T02:15:00+00:00",
     "output_chars": 800, "result_files": [], "measurement_source": "wall-clock", "dur_minutes": 15.0,
     "measurement_flags": []},
    {"run_id": "run-2026-09-26-impact-assessment-903", "role": "impact-assessment", "status": "queued",
...
FIXTURE_FILES = _exempted_local({
    "run-2026-09-26-code-review-901": {"code-review.report.md":
        "# 合成评审报告（fixture）\n\nRound 1：越查样本，用于核对 rounds 与报告轮次。\n"
        "Round 2：第二轮同样合成。\n"},
    "run-2026-09-26-impact-assessment-902": {"notes.md": "合成工作笔记（无轮次标记 → 报告不可数）\n"},
})
...
def legacy_rows(count: int) -> list[dict]:
    """合成**历史**（≤ 截止日）零时长 run：用于验证棘轮计数上限两个方向（含在限内/超限）。"""
    rows = []
    for i in range(1, count + 1):
        rid = f"run-2026-09-20-impact-assessment-8{i:02d}"
        rows.append({"run_id": rid, "role": "impact-assessment", "status": "succeeded",
...
def fixture(kind: str) -> tuple[list[dict], dict[str, dict[str, str]]]:
    ...
    if kind == "same-timestamp":  # ① 相同时间戳（dur 恰 0.00 → 退化：只标注、不写进测量字段）
        rows[0] = {**rows[0], "ended_at": rows[0]["started_at"], "measurement_source": "declared",
                   "dur_minutes": None, "measurement_flags": ["zero_duration"]}
    elif kind == "missing-dir":  # ② 缺目录（有记录无目录）
        files.pop("run-2026-09-26-code-review-901", None)
    elif kind == "rounds-mismatch":  # ③ rounds 与报告不符（报告 2 轮，账本声明 4）
        rows[0] = {**rows[0], "rounds_count": 4}
    elif kind == "terminal-not-written-back":  # ④ 终态未写回（queued 但目录里已有终态报告）
        files["run-2026-09-26-impact-assessment-903"] = {"impact-assessment.report.md": "# 已完成的报告\n"}
    elif kind == "orphan-dir":  # A 的另一向：有目录无记录
        files["run-2026-09-26-impact-assessment-904"] = {"impact-assessment.report.md": "# 孤儿目录\n"}
...
def write_fixture_root(base: Path, rows: list[dict], files: dict[str, dict[str, str]]) -> None:
    """把 fixture 写成 agents 根（`runtime/registry.json` + `runs/<run_id>/...`）。样本一律在 %TEMP%。"""
...
def report(policy, runs: list[dict], runs_dir: Path, problems: list[str], warnings: list[str],
    ...
    caps = policy.ledger_caps()
    cutoff = policy.ledger_cutoff_local_date()
    print(f"-- {label}：agents 根={runs_dir.parent}｜账本 {len(runs)} 条｜目录 "
          f"{len([p for p in runs_dir.iterdir() if p.is_dir()]) if runs_dir.is_dir() else '-'} 个｜"
          f"截止日 {cutoff}（含）以前=历史（棘轮）")
    ...
    if problems:
        ...
        for item in problems:
            print(f"  - {item}")
        return 1
    print(f"LEDGER-MEASUREMENT PASS：{label}（历史债 {sum(counts.values())} 例全部在上限内，新 run 零违规）")
    return 0
...
def exceptions_path(agents_root: Path, policy) -> Path:
    ...
    `ledger_measurement.run_dir_exceptions_file` 指向的文件；而反向对照样本（`%TEMP%` 下的
...
def selfcheck(policy) -> None:
    """内置自检：干净 fixture 必须过 + 四类反向对照必须 FAIL + 棘轮/白名单/契约的守卫自检。"""
    today = datetime.now(PROJECT_TZ).strftime("%Y-%m-%d")
    with tempfile.TemporaryDirectory(prefix="verify_ledger_measurement_") as td:
        ...
        ok("自检 ① 干净 fixture（截止日之后的新 run，字段齐全）→ 零 problem",
           problems == [], f"problems={problems[:2]} counts={counts}")

        for kind, expect, name in (
            ("same-timestamp", "zero_duration", "② 相同时间戳（dur 恰 0.00）"),
            ("missing-dir", "records_without_dir", "③ 缺目录（有记录无目录）"),
            ("rounds-mismatch", "rounds_mismatch", "④ rounds 与报告轮次不符"),
            ("terminal-not-written-back", "terminal_not_written_back", "⑤ 终态未写回（running/queued 却已有报告）"),
        ...
        problems, _, _ = evaluate_fixture(
            ...
            dir_exceptions=[{"dir": "run-2026-09-26-code-review-901", "reason": "过期例外样本：该目录已是账本 run"}])
        ...
        # ⑫⑬ 棘轮两个方向：限内只 WARN、超限 FAIL
        problems, warnings, counts = evaluate_fixture(policy, "legacy-within-cap", tmp / "cap-within", today=today)
        ok("自检 ⑫ 历史缺陷**在棘轮上限内**（3 = 上限）→ 不 FAIL，但逐条 WARN 点名（棘轮真的在放行，不是永远红）",
           problems == [] and counts["zero_duration"] == 3 and len(warnings) == 3,
           f"problems={problems[:1]} counts={counts} warns={len(warnings)}")
        problems, _, counts = evaluate_fixture(policy, "legacy-overflow", tmp / "cap-overflow", today=today)
        ok("反向对照 ⑬ 历史缺陷**超上限**（4 > 3）→ FAIL 且点名条数与上限",
           any("4 例 > 上限 3" in p for p in problems), f"problems={problems[:1]}")

        # ⑭-⑳ 新 run 的测量契约（每一类都必须能被单独命中）
...
def main() -> int:
    ...
    code = real_data(ROOT / "agents", policy)
    selfcheck(policy)
