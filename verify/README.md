...
| `verify_agentops.py` | Sprint-8/A-UC：AgentOps 账本 CLI 用例断言（**UC 用例号与断言数以该脚本现跑输出为准，本行不写死区间**；已含 TG-11 评审 scope 来源闸门 UC-14、M10 UC-11/12、M9 UC-13、TG-8 离线开关 UC-19、TG-19 M-A 与 D3 复核新增 UC-22/UC-23；隔离到临时 `AGENT_OPS_DIR`） | 无 API 调用，纯离线 |
...
| `verify_config_schema.py` | Sprint-11/13/F2：配置 SSOT 一致性断言——schema 结构/默认值/pydantic_path、validate_config 行为、**M7 前端零硬编码**（App.jsx n1 **不得含任何配置键字面量**——守卫键集合**由 `get_config_schema()` 派生、覆盖全部 schema 字段**，不再写死键名清单；字段总数以该脚本 `PASS: schema 字段总数` 行现跑为准）、**Settings 升级基线护栏**（字段路径 vs `settings_baseline.json`，`--regen-baseline` 重建） | 无 API 调用，纯离线 |
...
| `verify_ledger_measurement.py` | Sprint-17/TG-13：**账本『测量化』闸门**——口径可核（`dur`=墙钟累计/`rounds` 真源=报告轮次/缺值必须显式 `unknown`，真源 `agents/policy.json::ledger_measurement`）+ 账本↔`agents/runs/**` **双向**一致（白名单 `agents/policy/run-dir-exceptions.json`，每条须写 reason 且只减不增）+ 时间戳退化（相同/`0.00`/整十分钟/负值/缺失）+ `rounds` vs 报告轮次 + **终态已写回** + `measurement_source`/`dur_minutes` 契约 + **历史棘轮**（计数上限 + `review_by`，截止日之后一律严格）；默认模式 = 真实账本 + 30 条自检（含四类反向对照样本）。**账本缺失 ≠ 坏账本（C3，2026-09-25 code-review-072）**：`agents/runtime/registry.json` 被 `.gitignore` 忽略 ⇒ **全新 checkout 必然没有它**——此时打印 `SKIP: 账本不存在（fresh clone；registry.json 被 .gitignore 忽略）→ 该闸门只在本机/有账本的环境执行` 并 **rc=0**（否则 CI 的 offline 套件恒红）；文件**存在但坏**（JSON 非法 / 缺 `runs` / 结构非法）**照旧 rc=2 fail-closed**（坏账本是真缺陷，不静默放行） | **offline 档**；`python verify\verify_ledger_measurement.py`（真实账本 + 自检；无账本 → 显式 SKIP rc=0）；反向对照样本：`--emit-fixture <kind> --agents-root %TEMP%\tg13-rc\<kind>` 后用 `--agents-root` 判定（样本不写进仓库） |
...
| `gui_check_s15_*.mjs`（脚本个数以 `python verify\verify_matrix.py check` 与 `verify/TEST-MATRIX.MD` 现跑清单为准，本文件不写死） | Sprint-15 F2 验收修复族：`typography`（字体统一）/`hints_title`（聚合+限高）/`local_dir`（条件隐藏）/`collapse`（完成后收起）/`status_scope`（状态作用域）/`responsive`（三档分辨率）/`bidi_link`（双向联动）/`error_copy`（复制+报错定位 19 断言）/`output_view`（output/答案全文）/`cursor`（光标不跳+改动计数） | 后端 8787 + 前端 5173 已启动 + Playwright（`output_view` 为 network 档，需 key） |
| `verify_md_tables.py` | **TG-15/Sprint-17（2026-09-23 三次 R1 事故后固化）**：**Markdown 表格结构自检**——① 在**未转义** `\|` 处切分的单元格数一致性（裸管道由 ① 覆盖；② 的恒真式判据已删并由脚本内建 `reverse_control()` 坏/好样本对照证明删得对）② **扫描集双向差集**（政策 `md_table_coverage.roots`+`include_files` ↔ 文件系统，"应扫/实扫"必须相等——"漏扫"比"漏报"更危险）③ 棘轮上限（`md_table_legacy_files`）；**只报不改** | **offline 档**；`python verify\verify_md_tables.py` |
| `verify_card_index.py` | **TG-14④/Sprint-17**：**卡索引 lint**（"一卡一文档"从约定变成可判定不等式）——① `backlog.MD` 索引行 ↔ `cards/<卡号>.md` **双向**一致（孤儿行/孤儿文件/重号）② 卡号 ↔ 文件名（`card:` 字段）一致 ③ 卡文件必备节（`状态`/`规模`/`来源`/`Sprint`）④ **Sprint 文档不得复制卡正文**（整段行级指纹，防"只按卡号引用"退化）⑤ 表格结构（复用 `verify_md_tables.py` 判据）⑥ **状态三处同词**（卡↔索引↔Sprint 看板）⑦ 卡库存基线 `totals` 与 `scripts/card-inventory.py` **实测**一致；含反向对照自检 | **offline 档**；`python verify\verify_card_index.py`（默认档自 2026-09-25 起 = `--strict`） |
| `verify_gate_integrity.py` | **Sprint-17 关闭期交付／Sprint-18 扩展**：**闸门可信度**——① `guard_required` 内的闸门必须有 `-O`/`PYTHONOPTIMIZE` 守卫行（含**真实 `-O` 探针必须红**：裸 `assert` 承担判定时 `-O` 会让整套闸门空转并打印 `ALL PASS`）② **棘轮上限只许下调**（与 `git HEAD` 版逐项比较，上调即 FAIL）③ `review_by` 必填且未过期 ④ 未覆盖闸门逐条 WARN；另有"不可跳过层"**接线判据**（CI 调用必须在**命令位置**，注释/日志标签不算）；配置来自 `agents/policy.json::gate_integrity`。**读数（`guard_required`/`ratchets` 个数）一律以本脚本 `EVIDENCE:` 行现跑为准，不要手抄** | **offline 档**；`python verify\verify_gate_integrity.py` |
| `verify_derived_numbers.py` | **一查 finding 1/2 后交付（Sprint-17 关闭期）**：**派生数字不得手抄**——按文件棘轮约束"现测/实测 <数字>"式**当期主张**（断言数/卡数/文件数/脚本数/字节/C 级条数/棘轮计数）；合法形态（条文见 `1-WORKFLOW.MD` §6）：**复现命令 + "以该命令输出为准"**、**带日期/sha 的时点读数**（且同行可见）。生成物（`verify/TEST-MATRIX.MD`）豁免；上限相对 `git HEAD` **只减不增**；`review_by` 到期即 FAIL；`baseline` 死键即 FAIL；配置来自 `agents/policy.json::derived_numbers` | **offline 档**；`python verify\verify_derived_numbers.py`（`--report` 打印逐文件命中数，维护基线用；`--selftest` 只跑 fixture 反向对照） |
| `verify_close_tax_reading.py` | **TG-19 M-E/Sprint-18**：**关闭税读数机器派生**——从 `code-review` 报告 `## Graded findings` 与 `doc-audit` 报告 `## Must fix`+`## Should fix` **重新派生** `code_major`/`code_critical`/`doc_items`，与 Sprint 文档的「`关闭税读数:`」**机读行**逐字比较（治"手抄上一个 Sprint 的数字且算术自洽"）。退出码：不一致 ⇒ FAIL rc=1（逐项点名差值）；文档不可解析 ⇒ rc=2；**缺读数行**：文档自称已关闭 ⇒ FAIL rc=1，未自称关闭 ⇒ 具名 SKIP rc=0（**不是通过**）；读数行缺 `来源=` ⇒ FAIL；报告目录不存在 ⇒ 具名 SKIP（**不是通过**） | **offline 档**；`python verify\verify_close_tax_reading.py --sprint <Sprint 文档>`（无参 = 自检档 rc=0，断言数以末行现跑为准） |
| `verify_decision_register.py` | **TG-20 ③④⑤/Sprint-18**：**决策登记册机检**（`docs/6-DECISIONS.md` 是用户裁决的唯一登记处）——① 章节在位 ② 每个 `### <条目号>` 条目的字段**按 `§1 字段定义`全量**齐全（缺一即 FAIL，值与标签分开判） ③ `生效状态` 值必须以受控状态词开头 ④ `[已被 X 取代]` 的 `X` 必须真的存在（悬空取代即 FAIL）⑤ 全文 `path:line` **指针可核**（路径存在且行号在范围内；与"文件名重复/清点"类判据区分）⑥ §4.1 引文 ↔ 出处**逐条**核对 ⑦ "待回填"可见（不静默当合格）；含反向对照自检 | **offline 档**；`python verify\verify_decision_register.py` |

> **本表的登记口径**：脚本**全集与 tier** 的 SSOT 是 [`TEST-MATRIX.MD`](TEST-MATRIX.MD)（由各脚本头部 `VERIFY_META` 派生）；**本表是否漏登**以 `python verify\verify_matrix.py check` 与矩阵清单现跑逐项比对为准——**本文件不写死脚本数/闸门数/用例数/断言数**（这类计数一律以相应命令的现跑输出为准，见 `1-WORKFLOW.MD` §6"派生数字不得手抄"）。
