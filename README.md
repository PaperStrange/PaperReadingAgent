...
| [`docs/6-DECISIONS.md`](docs/6-DECISIONS.md) | **决策登记册**：clarification questions 与用户裁决的唯一登记处（原文逐字 ／ 生效状态 ／ 证据 ／ 验证时间戳 ／ 违背事故引用）；提问纪律见 `docs/1-WORKFLOW.MD` §6 第 9 条 |
...
2. 点击左侧 **1) Config** 节点，确认参数（也可用节点内**全字段配置表单**（SchemaForm，schema 驱动）下拉/输入选择，自动带出相关字段）：
   ...
   - **改参数后直接运行任意下游节点会自动先重跑 Config**（保证目录/模型等与当前配置一致）；retrieve 输出含 `result`（`ranked`=BM25 命中排序 / `fallback_first_n`=零命中回退前 N）与 `query_strategy`（含 CJK 的 query 零命中时自动提取英文关键词重试，`hit_note` 说明命中理由）。
...
- **用户裁决与生效状态（决策登记册）**：`docs/6-DECISIONS.md`
