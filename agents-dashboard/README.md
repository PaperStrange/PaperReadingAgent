...
│   └── api/                        # health / runs / runs/[id] / aggregates / specs / specs/[name] / specs/validate
...
- **仅回环 + 无鉴权**：`next start` 固定 `-H 127.0.0.1`（一键脚本与手动命令一致），spec 写端点不鉴权——本机信任模型，不对外网暴露；如需局域网访问请自行加反向代理鉴权。
